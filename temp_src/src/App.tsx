...
function FloatingSidePromos() {
  const [location] = useLocation();
  // Changed: Start TRUE (Visible)
  const [isVisible, setIsVisible] = useState(true);

  // Changed: Fade away after 5 seconds
  useEffect(() => {
  ...
  return (
    <>
      ...
      <motion.div
        ...
        className={`hidden lg:block fixed left-4 top-20 z-40 space-y-4 ${
      ...
      <motion.div
        ...
        transition={{ duration: 1 }}
        className={`hidden lg:block fixed right-4 top-20 z-40 space-y-4 ${
          isVisible ? "pointer-events-auto" : "pointer-events-none"
        }`}
      >
        <Tooltip>
