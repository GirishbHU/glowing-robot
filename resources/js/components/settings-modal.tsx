...
export default function SettingsModal({ isOpen, onClose }: SettingsModalProps) {
  ...
  const NameTypeSelector = ({
    ...
    <div className="bg-slate-800/50 rounded-xl p-4 border border-slate-700/50">
      ...
      <RadioGroup value={value} onValueChange={(v: string) => onChange(v as NameDisplayType)} className="space-y-1">
                ...
                >
                  {FANCY_NAME_SUGGESTIONS.slice(0, 8).map((name) => (
                    <button
                      ...
                      className={`px-2 py-1 rounded-full text-xs transition-all ${preferences.fancyName === name
